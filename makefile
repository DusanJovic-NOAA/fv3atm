...
FFLAGS   += -I$(FMS_INC) -Igfsphysics -Iipd -Icpl -Iio -Iatmos_cubed_sphere -Iccpp/driver -I../stochastic_physics
...
all: libs
	$(MAKE) $(FV3_EXE) $(MAKE_OPTS)

nems: libs
	$(MAKE) $(FV3CAP_LIB) $(MAKE_OPTS)
	$(MAKE) esmf_make_fragment

ifneq (,$(findstring CCPP,$(CPPDEFS)))
libs:
	$(MAKE) -C cpl                 $(MAKE_OPTS)
	$(MAKE) -C gfsphysics          $(MAKE_OPTS) 32BIT=N  DYN32=$(DYN32) # force gfs physics to 64bit, flag to CCPP build for 32bit dynamics
	$(MAKE) -C ccpp/driver         $(MAKE_OPTS) 32BIT=N  DYN32=$(DYN32) # force gfs physics to 64bit, flag to CCPP build for 32bit dynamics
	$(MAKE) -C ipd                 $(MAKE_OPTS) 32BIT=N  # force gfs physics to 64bit
	$(MAKE) -C io                  $(MAKE_OPTS)
	$(MAKE) -C atmos_cubed_sphere  $(MAKE_OPTS)
	$(MAKE) -C ../stochastic_physics  $(MAKE_OPTS) 32BIT=N  # force gfs physics to 64bit
...
libs:
	$(MAKE) -C cpl                 $(MAKE_OPTS)
	$(MAKE) -C gfsphysics          $(MAKE_OPTS) 32BIT=N  # force gfs physics to 64bit
	$(MAKE) -C ipd                 $(MAKE_OPTS) 32BIT=N  # force gfs physics to 64bit
	$(MAKE) -C io                  $(MAKE_OPTS)
	$(MAKE) -C atmos_cubed_sphere  $(MAKE_OPTS)
	$(MAKE) -C ../stochastic_physics  $(MAKE_OPTS) 32BIT=N  # force gfs physics to 64bit
